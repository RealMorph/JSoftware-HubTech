...
describe('AuthService - Brute Force Protection', () => {
  ...
  describe('CAPTCHA Challenge', () => {
    it('should implement CAPTCHA challenges for suspicious login attempts', async () => {
      ...
      service.login = jest.fn().mockImplementation(async (email: string, password: string, options?: any): Promise<EnhancedLoginResult> => {
      ...
      const captchaLoginResult = await service.login(
        testUserEmail,
        correctPassword,
        { captchaToken: 'valid-captcha-token-12345' }
      );
