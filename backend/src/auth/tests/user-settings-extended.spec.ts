...
import { NotificationFrequency, DashboardLayout, WidgetType } from '../dto/user-settings.dto';
...
describe('AuthService - Extended User Settings', () => {
  ...
  describe('Notification Frequency Settings', () => {
    it('should update notification frequencies', async () => {
      const frequencies = {
        email: NotificationFrequency.WEEKLY,
        push: NotificationFrequency.HOURLY,
        sms: NotificationFrequency.NEVER
      };
    ...
    it('should handle invalid frequency values', async () => {
      ...
      // The invalid value should be ignored and not updated
      const result = await service.updateNotificationFrequency(userId, frequencies);
      
      expect(result.frequencies.email).not.toBe('invalid');
      expect(result.frequencies.push).toBe(NotificationFrequency.HOURLY);
      expect(result.frequencies.sms).toBe(NotificationFrequency.NEVER);
    });
    ...
    it('should throw error for non-existent user', async () => {
      ...
      await expect(
        service.updateNotificationFrequency('non-existent-id', frequencies)
  ...
  describe('Notification Delivery', () => {
    ...
    it('should throw error for invalid notification channel', async () => {
      await expect(
        service.sendTestNotification(userId, 'invalid')
      ).rejects.toThrow(BadRequestException);
    });
    
    it('should throw error for non-existent user', async () => {
      await expect(
        service.sendTestNotification('non-existent-id', 'email')
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('Dashboard Settings', () => {
    it('should update dashboard layout', async () => {
      const layout = DashboardLayout.LIST;
    ...
    it('should add a dashboard widget', async () => {
      ...
      expect(result.widget.id).toBeDefined(); // Should generate a new UUID
    });
    ...
    it('should throw error when removing non-existent widget', async () => {
      await expect(
        service.removeDashboardWidget(userId, 'non-existent-widget-id')
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should validate widget positions when saving dashboard configuration', async () => {
        ...
        widgets: [
          {
            ...
            type: WidgetType.ACTIVITY,
            title: 'Recent Activity',
            position: { x: -1, y: 0, width: 2, height: 2 }, // Invalid: x < 0
            config: {}
          }
        ]
      };
      ...
      await expect(
        service.saveDashboardConfiguration(userId, invalidConfiguration)
      ).rejects.toThrow(BadRequestException);
    });
    
    it('should throw error for non-existent user', async () => {
      await expect(
        service.updateDashboardLayout('non-existent-id', DashboardLayout.LIST)
      ).rejects.toThrow(NotFoundException);
    });
  });
}); 