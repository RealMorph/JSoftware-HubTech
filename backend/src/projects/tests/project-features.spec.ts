...
describe('ProjectsService - Project Features', () => {
  ...
  describe('Project Tags Management', () => {
    ...
    it('should prevent adding duplicate tags', async () => {
      await expect(
        service.addProjectTag(projectId, { tag: 'initial-tag' }, userId)
    ...
    it('should throw error when removing non-existent tag', async () => {
      await expect(
        service.removeProjectTag(projectId, { tag: 'non-existent-tag' }, userId)
  ...
  describe('Project Files Management', () => {
    ...
    it('should throw error when retrieving non-existent file', async () => {
      await expect(
        service.getProjectFile(projectId, 'non-existent-id', userId)
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should update file metadata', async () => {
      ...
      expect(updatedFile.path).toBe(fileDto.path); // Unchanged
      expect(updatedFile.type).toBe(fileDto.type); // Unchanged
      expect(updatedFile.size).toBe(fileDto.size); // Unchanged
    });
    ...
    it('should delete a file', async () => {
      ...
      await expect(
        service.getProjectFile(projectId, addedFile.id, userId)
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('Project Activity Tracking', () => {
    it('should track project creation activity', async () => {
      // Activity is tracked automatically when a project is created
      ...
      // Find creation activity
      const creationActivity = activities.find(a => a.type === ActivityType.CREATED);
    ...
    it('should track project update activity', async () => {
      ...
      // Find update activity
      const updateActivity = activities.find(a => a.type === ActivityType.UPDATED && a.details.changedFields.includes('name'));
    ...
    it('should track tag activities', async () => {
      ...
      // Find tag addition activity
      const addTagActivity = activities.find(a => a.type === ActivityType.TAG_ADDED);
      ...
      // Find tag removal activity
      const removeTagActivity = activities.find(a => a.type === ActivityType.TAG_REMOVED);
    ...
    it('should track file activities', async () => {
      ...
      // Find file addition activity
      const addFileActivity = activities.find(a => a.type === ActivityType.FILE_ADDED);
      ...
      // Find file update activity
      const updateFileActivity = activities.find(a => a.type === ActivityType.FILE_UPDATED);
      ...
      // Find file deletion activity
      const deleteFileActivity = activities.find(a => a.type === ActivityType.FILE_DELETED);
    ...
    it('should limit number of activities returned', async () => {
      ...
      // Retrieve activities with limit
      ...
      expect(new Date(activities[0].timestamp).getTime()).toBeGreaterThanOrEqual(
