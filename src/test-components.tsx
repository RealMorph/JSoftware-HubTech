...
import { Table, TableHeader, TableBody, TableRow, TableCell, TableHeaderCell } from "./components/base/Table";
...
const TestComponents = () => {
  ...
  const validationRules = {
    ...
    password: [{ validator: (value: string) => value.length >= 6, message: "Password must be at least 6 characters" }]
  };
    ...
    <ThemeProvider themeService={inMemoryThemeService}>
      <div style={{ padding: "20px", maxWidth: "1200px", margin: "0 auto" }}>
        <div style={{ marginBottom: "20px" }}>
          <Button 
            onClick={() => { setShowForm(true); setShowDataDisplay(false); }}
          ...
          <Button 
            onClick={() => { setShowForm(false); setShowDataDisplay(true); }}
...
const FormContent = () => {
  ...
  const { values, errors, touched, handleChange: setFieldValue, handleBlur: setFieldTouched } = useForm();
