...
const BarRect = styled.rect<{active?: boolean}>`
  cursor: pointer;
...
const LinePoint = styled.circle<{active?: boolean}>`
  cursor: pointer;
...
const PieSlice = styled.path<{active?: boolean}>`
  cursor: pointer;
  transition: all 0.2s ease;
  stroke-width: ${props => props.active ? 2 : 1};
  stroke: ${props =>
      ...
      {showLegend && (
        <Legend>
          {data.map((point, i) => (
            ...
            >
              <LegendColor color={point.color || colorScale[i % colorScale.length]} />
              <span>{point.label} ({((point.value / total) * 100).toFixed(1)}%)</span>
            </LegendItem>
          ))}
        </Legend>
      )}
    ...
    >
      ...
      >
        <circle
          ...
          r={innerRadius * 50}
          fill="none"
          stroke={getThemeValue({} as ThemeConfig, 'colors.background')}
          strokeWidth="40"
        />
      </svg>
    </ChartContainer>
