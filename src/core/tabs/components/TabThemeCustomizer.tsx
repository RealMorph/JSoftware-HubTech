...
export const TabThemeCustomizer: React.FC<TabThemeCustomizerProps> = ({ onApplyTheme }) => {
  ...
  const [selectedTab, setSelectedTab] = useState<'default' | 'active' | 'hover' | 'animation'>(
    'default'
  );
  ...
  const previewStyles = {
    ...
    textTransform: tabStyles.textTransform as
      | 'none'
      | 'capitalize'
      | 'uppercase'
      | 'lowercase'
  ...
  return (
    <div className="tab-theme-customizer">
      ...
      <div className="tab-customizer-controls">
        <button className="tab-customizer-apply" onClick={handleApplyTheme}>
          Apply Theme
        </button>
        <button
