 ...
 */
export const TabConnector: React.FC<TabConnectorProps> = ({ tabManager, tabId, targetTabId }) => {
  const [message, setMessage] = useState('');
  const [stateUpdate, setStateUpdate] = useState('');

  // Use the messaging hook
  const { messages, lastMessage, sendMessage, sendEvent, clearMessages } = useTabMessaging(
    tabManager,
    tabId
  );

  // Use the state hook
  const { state, updateState, dependencies, addDependency, removeDependency } = useTabState(
    tabManager,
    tabId
  );
  ...
  const handleSendMessage = () => {
    if (message && targetTabId) {
      sendMessage(MessageType.CUSTOM_EVENT, { content: message }, targetTabId);
      setMessage('');
    }
  };
    ...
    <div className="tab-connector">
      ...
      <div className="dependencies-section">
        ...
        <ul>
          {dependencies.map((dep, index) => (
            <li key={index}>
              ...
              <button onClick={() => removeDependency(dep.providerId)}>Remove</button>
            </li>
          ))}
        </ul>
        ...
        {targetTabId && (
          <div className="dependency-controls">
            <button onClick={handleAddDependency}>Add Dependency to {targetTabId}</button>
            <button onClick={handleRemoveDependency}>Remove Dependency</button>
          </div>
        )}
      </div>
      ...
      <div className="messaging-section">
        ...
        <div className="message-list">
          ...
          {messages.length > 0 && <button onClick={clearMessages}>Clear Messages</button>}
        </div>
        ...
        {targetTabId && (
          <div className="message-input">
            ...
            />
            <button onClick={handleSendMessage}>Send to {targetTabId}</button>
          </div>
        )}
      </div>
    </div>
