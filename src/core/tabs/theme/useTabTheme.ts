...
import { defaultTabThemeExtension, TabStyleOptions, TabAnimationOptions, TabThemeExtension } from './tab-theme-extension';
...
export function useTabTheme() {
  ...
  const tabTheme: TabThemeExtension = {
    tabs: {
      styles: {
        default: {
          ...(theme?.currentTheme as any)?.tabs?.styles?.default || defaultTabThemeExtension.tabs.styles.default
        },
        active: {
          ...(theme?.currentTheme as any)?.tabs?.styles?.active || defaultTabThemeExtension.tabs.styles.active
        },
        hover: {
          ...(theme?.currentTheme as any)?.tabs?.styles?.hover || defaultTabThemeExtension.tabs.styles.hover
        }
      },
      animation: {
